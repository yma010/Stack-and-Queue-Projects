...
class MinMaxStack {
  ...
  min() {
    return (this.length > 0 ? this.minTrack[this.length - 1] : null);
  }

  max() {
    return (this.length > 0 ? this.maxTrack[this.length - 1] : null);
  }

  addNodeToTrackers(newNode) {
    if (this.max() === null || newNode.value > this.max().value) {
      this.maxTrack.push(newNode);
    } else {
      this.maxTrack.push(this.max());
    }

    if (this.min() === null || newNode.value < this.min().value) {
      this.minTrack.push(newNode);
    } else {
      this.minTrack.push(this.min());
    }
  }
  ...
  push(val) {
    ...
    this.addNodeToTrackers(newNode);
    return ++this.length;
  }
